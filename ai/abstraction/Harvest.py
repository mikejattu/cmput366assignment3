...
class Harvest(AbstractAction):
    ...
    def completed(self, gs):
        return gs.getPhysicalGameState().getUnit(self._target.getID())==None;
    ...
    def execute(self,  gs :GameState,  ru : ResourceUsage)->UnitAction :
        ...
        if (self._unit.getResources()==0) :
            ...
            if self._target.getX() == self._unit.getX() and self._target.getY() == self._unit.getY()-1 :
                return  UnitAction.build_Haverst(UnitAction.getDIRECTION_UP());
            if self._target.getX() == self._unit.getX()+1 and self._target.getY() == self._unit.getY():
                return  UnitAction.build_Haverst(UnitAction.getDIRECTION_RIGHT());
            if self._target.getX() == self._unit.getX() and self._target.getY() == self._unit.getY()+1:
                return  UnitAction.build_Haverst(UnitAction.getDIRECTION_DOWN());
            if self._target.getX() == self._unit.getX()-1 and self._target.getY() == self._unit.getY():
                return  UnitAction.build_Haverst(UnitAction.getDIRECTION_LEFT());
        else :
            ...
            if self._base.getX() == self._unit.getX() and self._base.getY() == self._unit.getY()-1:
                return  UnitAction.build_Return(UnitAction.getDIRECTION_UP())
            if self._base.getX() == self._unit.getX()+1 and self._base.getY() == self._unit.getY():
                return  UnitAction.build_Return(UnitAction.getDIRECTION_RIGHT())
            if self._base.getX() == self._unit.getX() and self._base.getY() == self._unit.getY()+1:
                return  UnitAction.build_Return(UnitAction.getDIRECTION_DOWN());
            if self._base.getX() == self._unit.getX()-1 and self._base.getY() == self._unit.getY():
                return  UnitAction.build_Return(UnitAction.getDIRECTION_LEFT());
